...
import utils
from models import *
from losses.binary import BinaryClassCriterion
import argparse
import yaml
import numpy as np
...
exec(dataset_exec)
training_root, testing_root = config_path(task=Task)
...
def train(epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        for sub_batch_idx in range(sub_batch_len):
            ...
            criterion = BinaryClassCriterion('MixedDiceBCE')
...
def test(epoch):
    ...
    with torch.no_grad():
        for idx, sample_name in enumerate(os.listdir(os.path.join(testing_root))):
            ...
            pred_volume = pred_volume > 0 # >0 is cancer <=0 is not cancer
            ...
            evaluator_dice.add_volume(pred_volume, gt_volume)
